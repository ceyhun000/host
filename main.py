...
@client.on(events.NewMessage(pattern=r"\.sbuludu$"))
async def wordcloud(event):
    ...
    try:
        ...
        messages = [message.message for message in history.messages if message.message]
        text = " ".join(messages)
        ...
        img.name = "wordcloud.png"  
        await client.send_file(event.chat_id, img, caption="**Söz buludu**")

        await event.reply("**Söz buludu hazırdır!**")
